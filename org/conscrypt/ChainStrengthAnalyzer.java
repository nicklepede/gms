    ...
    public static final void check(List list) {
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                throw new CertificateException("Unacceptable certificate: ".concat(String.valueOf(String.valueOf(x509Certificate.getSubjectX500Principal()))), e);
            }
        }
    }
    ...
    private static void checkKeyLength(X509Certificate x509Certificate) {
        ...
        if (publicKey instanceof ECPublicKey) {
            ...
            if (!(publicKey instanceof DSAPublicKey)) {
                throw new CertificateException("Rejecting unknown key class ".concat(String.valueOf(publicKey.getClass().getName())));
            }
    ...
    private static void checkSignatureAlgorithm(X509Certificate x509Certificate) {
        ...
        for (String str : SIGNATURE_ALGORITHM_OID_BLACKLIST) {
            if (sigAlgOID.equals(str)) {
                throw new CertificateException("Signature uses an insecure hash function: ".concat(String.valueOf(sigAlgOID)));
            }
        }
    }
